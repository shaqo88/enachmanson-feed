...
def get_raw_feed() -> str:
    """Use the exact snapshot check_feed.py already fetched and hashed,
    so check/update never disagree about what the 'current' feed is.
    Falls back to a live fetch for manual/local runs where no snapshot exists."""
    if os.path.exists(RAW_FEED_FILE):
        print(f"Using shared feed snapshot from {RAW_FEED_FILE} (same fetch check_feed.py compared)")
        with open(RAW_FEED_FILE, "r", encoding="utf-8") as f:
            return f.read()
    print(f"No snapshot found — fetching {PODBEAN_FEED_URL} directly ...")
    return fetch_feed(PODBEAN_FEED_URL)
